...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state) {
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/playground?error=missing_params`)
  }
  ...
  if (!clientId || !clientSecret) {
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/playground?error=oauth_not_configured`)
  }
  ...
  try {
    ...
    return NextResponse.redirect(`${process.env.NEXT_PUBLIC_APP_URL}/playground?error=oauth_failed`)
  }
}
